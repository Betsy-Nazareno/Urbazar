...
const Perfil = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost/personas/${localStorage.getItem('userId')}`).then((result) => {
      setUser(result.data)
      setLoad(true)
    })
  }, [])
  useEffect(() => {
    if (query.get('email')) {
      axios.get(`http://localhost/personas/${localStorage.getItem('userId')}`).then((result) => {
        ...
        axios.put(`http://localhost/personas/${localStorage.getItem('userId')}`, persona).then((result) => {
